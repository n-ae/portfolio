...
local function get_test_files(mode)
	local files = {
		quick = { "tests/functional/basic-xml-structure.xml" },
		comprehensive = {
			"tests/functional/basic-xml-structure.xml",
			"tests/performance/medium-test.xml",
			"tests/performance/large-test.xml",
		},
		benchmark = {
			"tests/functional/basic-xml-structure.xml",
			"tests/performance/medium-test.xml",
			"tests/performance/large-test.xml",
			"tests/performance/enterprise-scale-benchmark.xml",
			"tests/performance/large-benchmark.xml",
			"tests/performance/massive-scale-benchmark.xml",
		},
	}
...
local function setup_git_comparison(base_ref, implementations)
	...
	if
		os.execute("test -d " .. temp_dir .. "/fixml") == 0
		or os.execute("test -d " .. temp_dir .. "/fixml") == true
	then
	...
	for _, build_info in ipairs(build_commands) do
		...
		if
			os.execute("test -d " .. fixml_root .. "/" .. dir) == 0
			or os.execute("test -d " .. fixml_root .. "/" .. dir) == true
		then
			print("    Building " .. lang .. "...")
...
local function main()
	...
	if compare_base then
		...
		if git_data then
			print(
				"Git comparison setup complete: "
					.. #git_data.base_implementations
	...
	print(string.rep("=", 50))
	print(string.format("Mode: %s | Iterations: %d | Implementations: %d", mode, iterations, #implementations))
	if git_data then
	...
	for _, test_file in ipairs(test_files) do
		...
		for rank, result in ipairs(file_results) do
			...
			if avg_time == math.huge then
				...
				if mode == "benchmark" then
					print(string.format("    %d. %-12s %6.2fms  (%4.2fx)", rank, name, avg_time, speedup))
				else
					print(string.format("    %d. %-12s %6.1fms  (%4.1fx)", rank, name, avg_time, speedup))
				end
			end
		end
	...
	if git_data and #git_data.base_implementations > 0 then
		...
		for _, impl in ipairs(build_config.verify_implementations()) do
			...
			if current_stats and base_stats then
				...
				print(
					string.format(
						"  Current (%s):  %6.2fms avg (σ=%.2fms)",
						git_data.current_branch,
						current_stats.avg,
						current_stats.consistency
					)
				)
				print(
					string.format(
						"  Base (%s):     %6.2fms avg (σ=%.2fms)",
						compare_base:sub(1, 8),
						base_stats.avg,
						base_stats.consistency
					)
				)
	...
	print(string.format("Benchmark complete! Tested %d implementations successfully.", #implementations))
	return 0
end
