    ...
    if benchmark_mode and is_benchmark then
        ...
        if both_passed then
            if not csv_mode then
                ...
                local scan_ratio = rgcidr_result.execution_time_microseconds / grepcidr_result.execution_time_microseconds
                local full_ratio = rgcidr_result.execution_time_with_output_microseconds / grepcidr_result.execution_time_with_output_microseconds
                
                local pass_symbol = "✓"
                local warning = ""
                
                -- Check for statistical significance
                local max_cv = math.max(rgcidr_result.scan_cv, grepcidr_result.scan_cv, rgcidr_result.full_cv, grepcidr_result.full_cv)
                if max_cv > 5.0 then
                    pass_symbol = "⚠"
                ...
                print(string.format("  Scanning only: rgcidr: %s (%.1f%% var), grepcidr: %s (%.1f%% var) [%.2fx]", 
                    rgcidr_scan, rgcidr_result.scan_cv, grepcidr_scan, grepcidr_result.scan_cv, scan_ratio))
                print(string.format("  With output:   rgcidr: %s (%.1f%% var), grepcidr: %s (%.1f%% var) [%.2fx]", 
                    rgcidr_full, rgcidr_result.full_cv, grepcidr_full, grepcidr_result.full_cv, full_ratio))
            end
            passed = passed + 1
        else
            if not csv_mode then
                ...
                if not rgcidr_result.test_passed then
                    print("  rgcidr - Expected exit code: " .. rgcidr_result.expected_exit_code .. ", got: " .. rgcidr_result.actual_exit_code)
                end
                if not grepcidr_result.test_passed then
                    print("  grepcidr - Expected exit code: " .. grepcidr_result.expected_exit_code .. ", got: " .. grepcidr_result.actual_exit_code)
                end
            end
            failed = failed + 1
        end
...
local function run_tests()
    ...
    if csv_mode then
        ...
        for _, result in ipairs(csv_results) do
            if benchmark_mode then
                local time_str = result.execution_time_microseconds and string.format("%.0f", result.execution_time_microseconds) or ""
                print(string.format("%s,%s,%s,%s,%s", result.uat, result.test_scenario, result.category, result.result, time_str))
            else
